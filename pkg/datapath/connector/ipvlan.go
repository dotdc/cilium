...
// Returns the the tail call map, and an error if any operation while
// configuring said namespace fails.
func ConfigureNetNSForIPVLAN(netNsPath string) (*ebpf.Map, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Unable to open container netns %s: %s", netNsPath, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Unable to find ipvlan slave in container netns: %s", err)
	}

	m, err := setupIpvlanInRemoteNs(netNs, ipvlanIface, ipvlanIface)
	if err != nil {
		return nil, fmt.Errorf("Unable to setup ipvlan slave: %s", err)
	}
